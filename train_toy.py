...
class Trainer:
    ...
    def _run_epoch(self, epoch):
        ...
        for idx in self.train_data:
            ...
            print(
                f"[GPU{self.device.index}] Epoch {epoch} | Iteration: {batch_number}/{len(self.train_data)} | Loss: {self.loss}"
            )
            batch_number += 1
            self.log.append(
                f"[GPU{self.device.index}] Epoch {epoch} | Iteration: {batch_number}/{len(self.train_data)} | Loss: {self.loss}"
    ...
    def _save_checkpoint(self, epoch):
        ...
        torch.save(ckp, PATH)
        PATH_LOG = self.save_dir + f"/log_ep{epoch}.txt"
